...
export default function SearchableIndex() {
          ...
          observer = new IntersectionObserver(
            (entries) => {
              if (
                ...
                visibleCharacters().length < filteredCount()
              ) {
                setPage((p) => p + 1);
              }
            },
  ...
  createEffect(() => {
    ...
    if (query) {
      filtered = filtered.filter((c) => {
        const matchChar = c.unicode.includes(query);
        const matchDef = c.english_definition?.toLowerCase().includes(query);
        const matchReading = c.gong_huangcheng_reading
          ?.toLowerCase()
          .includes(query);
    ...
    if (quality === "missing") {
      filtered = filtered.filter(
        (c) =>
          !c.english_definition?.trim() || !c.gong_huangcheng_reading?.trim(),
      );
      ...
      filtered = filtered.filter(
        (c) => c.english_definition?.toLowerCase() === targetDef,
      );
    }
  ...
  createEffect(() => {
    if (isPopState || !initialLoadComplete()) {
      if (isPopState) isPopState = false;
      return;
    }
      ...
      >
        ...
        <div class="mb-4">
          <label for="search-input" class="sr-only">
            Search characters, readings, or definitions
          </label>
          <div class="relative">
            <input
              ref={searchInputRef}
        ...
        <div class="mb-4 flex flex-wrap gap-3 items-center">
          <div class="flex items-center gap-2">
            <label
              for="filter-quality"
              class="text-sm font-ui text-brown-700 dark:text-beige-400"
            >
              Data:
            </label>
            <select
              ...
              value={dataQuality()}
              onChange={(e) => {
                setDataQuality(e.currentTarget.value as DataQualityFilter);
                setPage(0);
              }}
          ...
          <div class="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                setSelectedInitial(e.currentTarget.value);
                setPage(0);
              }}
              class="px-2 py-1.5 text-sm font-ui bg-beige-100 dark:bg-brown-800 border border-brown-400 dark:border-brown-600 text-brown-900 dark:text-beige-100 focus:outline-none focus:ring-2 focus:ring-brown-900 dark:focus:ring-beige-100"
            >
          ...
          <div class="flex items-center gap-2">
            ...
            <select
              ...
              value={selectedRhyme()}
              onChange={(e) => {
                setSelectedRhyme(e.currentTarget.value);
                setPage(0);
              }}
              class="px-2 py-1.5 text-sm font-ui bg-beige-100 dark:bg-brown-800 border border-brown-400 dark:border-brown-600 text-brown-900 dark:text-beige-100 focus:outline-none focus:ring-2 focus:ring-brown-900 dark:focus:ring-beige-100"
            >
                ...
                >
                  <div class="font-reading text-sm text-brown-600 dark:text-beige-400">
                    {char.gong_huangcheng_reading
                      ? `${char.gong_huangcheng_reading}${tone()}`
                      : "—"}
                  </div>
